...
ESM-only TypeScript library. Node.js 20+. Package manager is **pnpm** (`packageManager` in `package.json`). No runtime dependencies — never add any.

## Commands

```bash
pnpm install
pnpm fixtures          # download SHA-pinned samples into test/fixtures/ (gitignored)
pnpm lint              # biome check
pnpm typecheck
pnpm test              # fixtures:check + tsc + node --test test/
pnpm check             # lint + typecheck + test
```

Run `pnpm fixtures` before tests. `pnpm test` runs `fixtures:check` as `pretest` and fails if any supported format lacks a verified on-disk sample with a decodable preview. Never commit `test/fixtures/` or `dist/`.

Finish work with `pnpm check` passing. Use Biome (`pnpm lint:fix`) rather than Prettier/ESLint.
...
7. `pnpm fixtures` then `pnpm check`.
